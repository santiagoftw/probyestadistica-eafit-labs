...
Replica a mano los 5 pasos: rango → k (Sturges) → ancho de clase → límites → conteo. La función `cut()` asigna cada dato a su intervalo, y `table()` cuenta:
...
```{r}
h <- hist(faithful$eruptions, breaks = k,
          col = "#004B85", border = "white",
          xlab = "Duración (min)", ylab = "Frecuencia",
          main = "hist() de R base")
```
...
tabla_tidy <- tibble(duracion = faithful$eruptions) |>
  mutate(clase = cut(duracion, breaks = limites, include.lowest = TRUE)) |>
...
ggplot(tabla_tidy, aes(x = clase, y = Frec_absoluta)) +
  ...
  labs(x = "Duración (min)", y = "Frecuencia",
...
ggplot(faithful, aes(x = eruptions)) +
  ...
  geom_vline(aes(xintercept = mean(eruptions)),
...
### Otro ejemplo: la tabla agrupada completa, paso a paso

El ejemplo anterior se detuvo en el histograma. Aquí completamos los 5 pasos hasta la tabla de frecuencias agrupada, con otro dataset — el peso (kg) de 30 estudiantes:

```{r}
pesos <- c(47,48,52,54,55,56,57,58,60,61,63,65,66,67,68,69,
           70,71,71,72,74,74,76,79,82,83,85,88,92,94)

# Paso 1-3: rango, k (Sturges) y amplitud de clase
rango  <- max(pesos) - min(pesos)
k      <- ceiling(1 + 3.3 * log10(length(pesos)))
amplitud <- rango / k
c(rango = rango, k = k, amplitud = round(amplitud, 2))

# Paso 4-5: límites e intervalos, y conteo
limites <- seq(min(pesos), by = ceiling(amplitud), length.out = k + 1)
clases  <- cut(pesos, breaks = limites, right = FALSE, include.lowest = TRUE)

tabla_pesos <- data.frame(
  Intervalo          = levels(clases),
  Frec_absoluta      = as.vector(table(clases)),
  Frec_acumulada     = cumsum(as.vector(table(clases))),
  Frec_relativa      = round(as.vector(table(clases)) / length(pesos), 2),
  Frec_rel_acumulada = round(cumsum(as.vector(table(clases))) / length(pesos), 2)
)
tabla_pesos
```
...
::: {.callout-note}
## Lo que deberías notar
Aquí se redondeó la amplitud **hacia arriba** (`ceiling(amplitud)`, de 7.83 a 8) antes de construir los límites, en vez de usar `seq(..., length.out = k+1)` con la amplitud exacta como en el ejemplo de Old Faithful. Es la diferencia que menciona la teoría de este tema: cuando el ejercicio pide límites "redondos" (aquí, intervalos de amplitud 8 en vez de 7.83), redondeas la amplitud antes de construir los cortes — el número de intervalos ($k$) puede quedar en 6 de cualquier forma, pero los límites concretos sí cambian.
:::
