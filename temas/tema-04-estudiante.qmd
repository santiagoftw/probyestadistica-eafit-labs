...
mean(tension); median(tension)  # deberían quedar muy cerca
...
# Caso 4: ancho de grietas en placas — asimetría positiva
grietas <- c(1.25, 1.31, 1.38, 1.48, 1.59, 1.63, 1.66, 1.70, 1.78, 1.81, 2.21)
mean(grietas); median(grietas)  # la media > mediana revela sesgo a la derecha
```

::: {.callout-important}
## Interpretación del Caso 4
Cuando la **media es mayor que la mediana**, es señal de asimetría positiva (sesgo a la derecha) — hay algunos valores grandes "jalando" la media hacia arriba, mientras la mediana se mantiene en el centro posicional de los datos.
:::
